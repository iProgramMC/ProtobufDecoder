...
namespace Protobuf
{
	...
	public:
		...
		// Checks if this object is a byte storage object
	...
	static void DecodeBlock(const char* data, size_t sz, ObjectBaseMessage* block)
	{
		size_t offset = 0;
		while (offset < sz)
		{
			...
			case TAG_LEN:
			{
				...
				else
				{
					...
					try {
						DecodeBlock(dataSubset, length, pChld);
